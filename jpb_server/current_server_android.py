...
# Android donor-cache default. It can still be overridden without editing:
#   JPB_MANIFEST_FILE=config/fixed_manifest.json
#   JPB_CACHE_DIRS=cache_files_guy_android;cache_files
MANIFEST_FILE = os.path.join(BASE_DIR, os.environ.get("JPB_MANIFEST_FILE", os.path.join("config", "fixed_manifest2.json")))
_CACHE_DIRS = os.environ.get("JPB_CACHE_DIRS", "cache_android")
CACHE_DIRS = [
    os.path.join(BASE_DIR, entry.strip())
    for entry in _CACHE_DIRS.split(";")
    if entry.strip()
]
CACHE_DIR = CACHE_DIRS[0] if CACHE_DIRS else os.path.join(BASE_DIR, "cache_android")
_LPKG_REWRITE_ENV = os.environ.get("JPB_REWRITE_LPKG_VERSION", "6").strip()
try:
    LPKG_REWRITE_VERSION = int(_LPKG_REWRITE_ENV)
except ValueError:
    LPKG_REWRITE_VERSION = 6
if not 0 <= LPKG_REWRITE_VERSION <= 255:
    LPKG_REWRITE_VERSION = 6
LPKG_REWRITE_VERSION = LPKG_REWRITE_VERSION or None
